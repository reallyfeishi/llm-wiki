...
while IFS= read -r -d '' file; do
  if ! grep -qP 'linked_from: \[.+' "$file" 2>/dev/null; then
    echo "- [孤立] $(basename "$file" .md)" >> "${REPORT_FILE}"
    ORPHAN_COUNT=$((ORPHAN_COUNT + 1))
  fi
done < <(find "${KB_PATH}/wiki" -name "*.md" -not -name "index.md" -not -name "log.md" -not -name "lint-report*" -print0)
...
while IFS= read -r -d '' file; do
  grep -oP '\[\[([^\]]+)\]\]' "$file" 2>/dev/null | while read -r link; do
    ...
    if [ $found -eq 0 ]; then
      echo "- $(basename "$file" .md) -> [[${slug}]] (不存在)" >> "${REPORT_FILE}"
    fi
  done
done < <(find "${KB_PATH}/wiki" -name "*.md" -not -name "index.md" -not -name "log.md" -not -name "lint-report*" -print0)
...
while IFS= read -r -d '' file; do
  ...
  if [ -n "$updated" ] && [[ "$updated" < "2026-04-27" ]]; then
...
# 统计摘要
TOTAL_PAGES=$(find "${KB_PATH}/wiki" -name "*.md" -not -name "index.md" -not -name "log.md" -not -name "lint-report*" | wc -l)
echo "---" >> "${REPORT_FILE}"
echo "" >> "${REPORT_FILE}"
echo "**摘要**: 共 ${TOTAL_PAGES} 个 wiki 页面，${ORPHAN_COUNT} 个孤立页面，${STALE_COUNT} 个过时页面，${UNPROCESSED} 个未消化源文档" >> "${REPORT_FILE}"
